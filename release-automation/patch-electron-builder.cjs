...
LinuxTargetHelper.prototype.computeMimeTypeFiles = async function (...args) {
    ...
    const xmlPath = pathUtil.join(__dirname, '../linux-files/org.nitrobolt.NitroBolt.mime.xml');
    const rawXml = await fsPromises.readFile(xmlPath, 'utf-8');
    const newXml = rawXml.replace(/org\.nitrobolt\.NitroBolt/g, packageJSON.name);
